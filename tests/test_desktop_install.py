...
def prepare_install(tmp_path):
    checkout = tmp_path / 'checkout SIT \' "$`\\ %f'
    home = tmp_path / 'home SIT \' "$`\\ %f'
...
def test_installer_replaces_known_legacy_desktop_entry(tmp_path):
    ...
    assert "Name=SIT\n" in (home / "data" / "applications" / "sit.desktop").read_text()


@pytest.mark.skipif(sys.platform != "linux", reason="Linux installer")
def test_installer_replaces_prior_shit_installation(tmp_path):
    _, home, _, env, command = prepare_install(tmp_path)
    legacy_launcher = home / ".local" / "bin" / "shit"
    legacy_entry = home / "data" / "applications" / "shit.desktop"
    legacy_icon = home / "data" / "icons" / "shit.png"
    ...
    legacy_icon.write_bytes(b"test icon")
    ...
    assert (home / "data" / "applications" / "sit.desktop").exists()
